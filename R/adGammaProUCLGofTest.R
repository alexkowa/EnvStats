...
function (x, distribution = c("gamma", "gammaAlt")) 
{
    ...
    n.vec <- dimnames(EnvStats::ProUCL.Crit.Vals.for.AD.Test.for.Gamma.array)[[1]]
    ...
    k.vec <- dimnames(EnvStats::ProUCL.Crit.Vals.for.AD.Test.for.Gamma.array)[[2]]
    ...
    crit.vals.names <- dimnames(EnvStats::ProUCL.Crit.Vals.for.AD.Test.for.Gamma.array)[[3]]
    p.vec <- as.numeric(substr(crit.vals.names, 3, nchar(crit.vals.names)))
    interp.array <- EnvStats::ProUCL.Crit.Vals.for.AD.Test.for.Gamma.array[row.index.n, 
    ...
    interp.fcn <- function(mat, n.vec, k.vec, n, k) {
        ...
        if (length.n.vec == 1 && length.k.vec == 1) {
            ...
            nchar.vec <- nchar(sapply(strsplit(sub("0+$", "", 
                as.character(mat)), ".", fixed = TRUE), function(x) x[2]))
            nchar.vec[is.na(nchar.vec)] <- 0
            digits <- min(nchar.vec)
            if (digits < 3) 
                digits <- 3
            if (length.n.vec == 1) {
    ...
    ad.crit.vals <- apply(interp.array, 3, interp.fcn, n.vec = n.vec, 
        k.vec = k.vec, n = n, k = shape)
    names(ad.crit.vals) <- crit.vals.names
    z <- pgamma(sort(x), shape = shape, scale = scale)
    ad.stat <- -n - (1/n) * sum((2 * (1:n) - 1) * (log(z) + log(1 - 
        rev(z))))
    names(ad.stat) <- "A"
    if (ad.stat > ad.crit.vals["A.0.01"]) 
        p <- "< 0.01"
    else if (ad.stat > ad.crit.vals["A.0.05"]) 
        p <- "0.01 <= p < 0.05"
    else if (ad.stat > ad.crit.vals["A.0.10"]) 
        p <- "0.05 <= p < 0.10"
    else p <- ">= 0.10"
    ret.list <- c(ret.list, list(statistic = ad.stat, parameters = n, 
        crit.vals = ad.crit.vals, p.value = p, alternative = paste("True cdf does not equal the\n", 
            space(33), ret.list$distribution, " Distribution.", 
            sep = ""), method = "ProUCL Anderson-Darling Gamma GOF", 
        data = x))
