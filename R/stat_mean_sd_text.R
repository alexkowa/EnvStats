...
#'   for each value on the \eqn{x}-axis, add text indicating the mean and
#'   standard deviation of the \eqn{y}-values for that particular \eqn{x}-value.
#' @usage
...
#'   mean and standard deviation.  When \code{digit.type="round"} (see below)
...
#' @rawRd
#' \note{
#'   The function \code{stat_mean_sd_text} is called by the function \code{\link{geom_stripchart}}.
#' }
#' @rawRd
#' \seealso{
#'   \code{\link{geom_stripchart}}, \code{\link{stat_median_iqr_text}},
    ...
    vjust = 0.5, ...) 
{
    ...
    ggplot2::layer(stat = StatMeanSDText, data = data, mapping = mapping, 
