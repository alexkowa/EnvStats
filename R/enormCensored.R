...
#' and background material are available in \code{vignette("extended-function-details", package = "EnvStats")}, section \code{enormCensored}.
#' @rawRd
#' \value{
...
enormCensored <-
    function(
        ...
        if (multiple) {
            ret.list <- do.call("enormMultiplyCensored", args = args.list)
        } else {
            ret.list <- do.call("enormSinglyCensored", args = args.list)
        }
