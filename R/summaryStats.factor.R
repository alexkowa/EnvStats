    ...
    ...)
{
    ...
    if (is.null.group || L0 || one.category) {
        mat <- summaryStats_factor_vec(x = object, combine.levels = combine.levels,
        ...
        levels.group <- levels(group)
        dum.list <- lapply(split(object, group), summaryStats_factor_vec,
        ...
        if (combine.groups) {
            Combined <- summaryStats_factor_vec(object, combine.levels = combine.levels,
                digits = digits, digit.type = digit.type, show.na = show.na,
                show.0.na = show.0.na)
