...
function (x, censored, censoring.side, left.censored.min, right.censored.max, 
    ...
    obs.mean) 
{
    ...
    for (i in 1:n.bootstraps) {
        ...
        if (new.n.cen == 0) {
            ...
            boot.vec[i] <- do.call(est.fcn, list(x = new.x, censored = new.censored, 
                ...
                right.censored.max = right.censored.max, ci = FALSE))$parameters["mean"]
        }
    }
    alpha <- 1 - conf.level
    ...
    if (use.acc.con) {
        jack.vec <- enparCensored.jackknife(x = x, censored = censored, 
            censoring.side = censoring.side, left.censored.min = left.censored.min, 
            right.censored.max = right.censored.max, est.fcn = est.fcn)
    ...
    ret.obj <- list(name = "Confidence", parameter = "mean", 
        ...
        conf.level = 1 - alpha, sample.size = N, n.bootstraps = n.bootstraps, 
